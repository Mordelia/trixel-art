...
def triangle_pixelate(input_path, output_path, triangle_size=20, use_dominant_color=False):
    """
    ...
    for pts in triangles:
        ...
        if len(pixels) == 0:
            ...
            if use_dominant_color:
                ...
                color = tuple(unique_colors[dominant_idx])
            else:
                # Use the mean color
                color = tuple(np.mean(pixels, axis=0).astype(int))
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print("Usage: python triangle_pixelate.py input_image output_image [triangle_size] [--dominant]")
        ...
        triangle_pixelate(input_path, output_path, triangle_size, use_dominant_color)
